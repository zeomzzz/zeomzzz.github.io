...
| --- | --- |
| 주 테이블에 외래 키
(주테이블 : 주로 access하는 테이블) | - 주 객체가 대상 객체의 참조를 가지는 것 처럼 주 테이블에 외래 키를 두고 대상 테이블을 찾음 <br> - 객체지향 개발자 선호 <br> - JPA 매핑 편리 <br> - 장점: 주 테이블만 조회해도 대상 테이블에 데이터가 있는지 확인 가능 <br> - 단점: 값이 없으면 외래 키에 null 허용 (-> DB 입장에서 별로..) |
