...
- `EntityManager.persist(entity);`
    ...
    - 실제로는 ..
        ...
        - persist : Entity를 영속성 컨텍스트라는 것에 저장 (DB에 저장 X)
...
- 애플리케이션과 DB 사이에 영속성 컨텍스트라는 중간 계층이 있는 것
...
- 조회할 때에 1차 캐시 확인 후, 없는 경우에 DB로 쿼리를 보냄
    ...
    - 과정 : 1차 캐시에서 찾아봄 &rarr; member2는 1차 캐시에 없었음 &rarr; JPA가 DB에서 조회 &rarr; DB의 member2를 2차 캐시에 저장 . 그리고 member2를 반환 &rarr; 이후에 member2를 조회하면 1차 캐시에서 찾아봄
...
> - 두번째 find에서 똑같은 것을 조회 &rarr; 1차 캐시부터 찾고, 찾은 것을 반환
...
- 내부 메커니즘
    - `em.persist(memberA);` : memberA가 1차 캐시에 들어가고, 동시에 JPA가 엔티티 분석해서 insert 쿼리 생성 &rarr; 쿼리를 쓰기 지연 SQL 저장소에 쌓음 (아직 DB에 넣지 X)
